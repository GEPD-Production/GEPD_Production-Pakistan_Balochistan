...
indicators <- list(
  ...
  c("SE.PRM.BFIN.4", "4*as.numeric(`Efficiency by the relationship between financing and outcomes; where 0 is the lowest possible efficiency and 1 is the highest`)+1", "finance", "NA", "NA"), # (Financing) Efficiency - Expressed by the score from the Public Expenditure and Financial Accountability (PEFA) assessment~
  c("SE.PRM.BFIN.3", "4*as.numeric(`Efficiency by the score from the Public Expenditure and Financial Accountability (PEFA) assessment; where 0 is the lowest possible efficiency and 1 is the highest`)+1", "finance", "NA", "NA"), # (Financing) Efficiency - Expressed by the relationship between financing and outcomes; where 0 is the lowest possible effi~
...
# Evaluate the indicator expression and check for Inf values
stat_df <- stat_df %>%
  mutate(VALUE = eval(parse(text = indicator)))

if (any(is.infinite(stat_df$VALUE))) {
  stop("The VALUE column contains Inf values after evaluation.")
}

# Filter out NA and Inf values in school_weight
stat_df <- stat_df %>%
  filter(!is.na(school_weight)) %>%
  filter(!is.infinite(school_weight))

# Check for Inf values in school_weight
if (any(is.infinite(stat_df$school_weight))) {
  stop("The school_weight column contains Inf values after filtering.")
}
...
result <- stat_df %>%
  ...
  ungroup() %>%
  summarise(
    mean = survey_mean(value, na.rm = TRUE),
    N = sum(!(is.na(value) | is.na(school_weight)))
  )
